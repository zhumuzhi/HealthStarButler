...
- (void)initSubViews
{
    ...
    self.controllerToolBar.backgroundColor = MEDGrayColor(170);
...
{
    //NSLog(@"查看默认字符串:%@", defaultStr);
    
    _defaultStr = defaultStr;
    
    self.stateStr = defaultStr;
    NSInteger seletIndex = 0;
    if ([_defaultStr isEqualToString:@"男"]) {
        seletIndex = 0;
    }else if ([_defaultStr isEqualToString:@"女"]) {
        seletIndex = 1;
    }else {
        seletIndex = [defaultStr integerValue];
    }
    
    [self.statePicker selectRow:seletIndex inComponent:0 animated:NO];
}
