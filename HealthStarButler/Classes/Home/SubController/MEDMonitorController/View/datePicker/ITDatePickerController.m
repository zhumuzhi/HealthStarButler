...
- (void)setMinimumDate:(NSDate *)minimumDate {
    ...
    if (minimumDate == nil) {
        minimumDate = [self.dateFormatter dateFromString:@"2010.01"];
    }
...
//        _cancelButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(cancelButtonOnClicked:)];
        ...
        [cancelButton setTitleColor:MEDGrayColor(102) forState:UIControlStateNormal];
        [cancelButton setTitle:@"取消" forState:UIControlStateNormal];
        [cancelButton addTarget:self action:@selector(cancelButtonOnClicked:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIBarButtonItem *)confirmButton {
    if (!_confirmButton) {
        ...
        [confirmButton setTitleColor:MEDCommonBlue forState:UIControlStateNormal];
        [confirmButton setTitle:@"确认" forState:UIControlStateNormal];
        [confirmButton addTarget:self action:@selector(confirmButtonOnClicked:) forControlEvents:UIControlEventTouchUpInside];
        _confirmButton = [[UIBarButtonItem alloc] initWithCustomView:confirmButton];
