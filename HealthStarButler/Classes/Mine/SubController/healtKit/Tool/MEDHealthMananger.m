...
#import "MEDHealthMananger.h"
#import "NSDate+TFDate.h"

@interface MEDHealthMananger ()
...
static MEDHealthMananger *_healthManager = nil;

@implementation MEDHealthMananger
...
+ (instancetype)TF_standardHealthManager {
    ...
    dispatch_once(&onceToken, ^{
        _healthManager = [[MEDHealthMananger alloc] init];
    });
...
+ (BOOL)TF_isAuthorizationStatusForType:(HKObjectType *)type {
    HKAuthorizationStatus  authorizationType = [[MEDHealthMananger TF_standardHealthManager].healthStore authorizationStatusForType:type];
...
+ (void)TF_authorizeHealthKitWithType:(TFQuantityType)type result:(void(^)(BOOL isAuthorizateSuccess ,NSError *error))resultBlock {
    ...
    //    static dispatch_once_t onceToken;
    //    dispatch_once(&onceToken, ^{
    [[MEDHealthMananger TF_standardHealthManager].healthStore requestAuthorizationToShareTypes:readSet readTypes:readSet completion:^(BOOL success, NSError * _Nullable error) {
        if (resultBlock) {
...
+ (void)TF_getStepCountWithHourOf0ClockBlock:(void (^)(NSArray *queryResults))queryResultBlock{
    ...
    for (NSArray *dArr in currentDateArray) {
        ...
        [[MEDHealthMananger TF_standardHealthManager].healthStore executeQuery:query];
    }
}
+ (NSArray *)getAllDate {
...
+ (void)TF_getBodyType:(HKSampleType *)sampleType queryResultBlock:(void (^)(NSArray *queryResults))queryResultBlock  {
    ...
    [[MEDHealthMananger TF_standardHealthManager].healthStore executeQuery:sampleQuery];
}
#pragma mark -- 写入数据
+ (void)TF_saveHealthKitDataType:(TFQuantityType )type
    ...
    [[MEDHealthMananger TF_standardHealthManager].healthStore saveObject:quantitySample withCompletion:^(BOOL success, NSError * _Nullable error) {
        block(success, error);
    }];
