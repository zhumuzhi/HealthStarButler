...
- (void)strokeStart
{
    ...
    CGFloat yLength = self.height - self.topGap - self.xLabelsHeight;
...
- (void)strokeYSepratorLine
{
    ...
    CGFloat startPointY = self.height-self.xLabelsHeight;
...
//    CGFloat x = self.width-self.yLabelWidth-Gap*2;
...
//        CGFloat yEndP = self.height-self.xLabelsHeight-(80.0f/self.maxYValue * self.sepYLength*self.ySepLabelCount)
    
    //矩形起点
    CGFloat rectX = 0;
    CGFloat rectYValue = value1;
    CGFloat rectY = self.height-self.xLabelsHeight-(rectYValue/self.maxYValue * self.sepYLength*self.ySepLabelCount);
    CGFloat rectYValue2 = value2;
    CGFloat rectY2 = self.height-self.xLabelsHeight-(rectYValue2/self.maxYValue * self.sepYLength*self.ySepLabelCount);
...
- (void)setupLegendWithlegends:(NSArray *)legends{
    ...
    CGFloat gapW = 0;
    CGFloat firstX = ((self.width-(legends.count*70)-gapW*gapCount)/2); //(图例宽-图例个数-i*图例间距)/2
    CGFloat itemY = 3;
    ...
    for (int i=0; i<legends.count; i++) {
        ...
        UILabel *legendLbl = [[UILabel alloc] initWithFrame:CGRectMake(legendItem.right , 0, 50, 20)];
...
- (void)setupUnitLanbel {
    ...
    //时间单位
    UILabel *dateLbl = [[UILabel alloc] initWithFrame:CGRectMake(self.width-20, self.height-(self.xLabelsHeight+2.5), 20, 20)];
    dateLbl.text = @"日";
...
{
    if (!_scrollView) {
        _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(self.yLabelWidth, 0, self.width-self.yLabelWidth-Gap*2, self.height)];
